    ...
    <div style={{ position: 'relative', width: '100%' }} onClick={fire}>
      {sparks.map(sp => (
        <span key={sp.id} style={{
          ...
          animation: `rgspk${Math.round(sp.angle)} 0.65s ease-out forwards`,
        }} />
      ))}
      <style>{`${Array.from({ length: 360 }, (_, i) => `@keyframes rgspk${i}{0%{opacity:1;transform:translate(-50%,-50%) scale(1);}100%{opacity:0;transform:translate(-50%,-50%) translate(${Math.cos(i * Math.PI / 180) * 72}px,${Math.sin(i * Math.PI / 180) * 72}px) scale(0);}}`).join('')}`}</style>
      {children}
    </div>
  );
};
    ...
    <>
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        .at-root { font-family:'Inter',sans-serif; color:white; position:relative; z-index:1; }
