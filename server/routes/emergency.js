...
router.post('/panic/trigger', async (req, res) => {
  try {
    ...
    const isStaffTriggered = user.role === 'committee' || user.role === 'admin';
    const visibility = isStaffTriggered ? 'all' : 'staff_only';
    ...
    res.status(201).json({
      ...
      visibility: isStaffTriggered ? 'Broadcast to everyone' : 'Visible to staff only',
      instructions: [
        '🚨 Emergency services have been notified',
        isStaffTriggered ? '📢 All users are being alerted' : '📢 Staff members are being alerted',
...
router.get('/panic/status', async (req, res) => {
  try {
    ...
    const isStaff = userRole === 'committee' || userRole === 'admin';
    ...
    // Students only see alerts visible to all
    if (!isStaff) {
...
// ===================================
// ROUTE 4: DELETE PANIC ALERT
// DELETE /api/emergency/panic/:id
// Protected - admin only
// ===================================
router.delete('/panic/:id', async (req, res) => {
  try {
    const user = getUserFromToken(req);
    if (!user) {
      return res.status(401).json({ message: 'Access denied. Please login.' });
    ...
    // Only admin can delete panic alerts
    if (user.role !== 'admin') {
    ...
    console.error('Panic delete error:', error.message);
    res.status(500).json({ message: 'Server error', error: error.message });
  }
});

// ===================================
// ROUTE 5: GET EVACUATION ROUTES
...
// ===================================
// ROUTE 6: CLEANUP RESOLVED ALERTS
// DELETE /api/emergency/panic/cleanup
// Protected - admin only
// ===================================
router.delete('/panic/cleanup', async (req, res) => {
  try {
    const user = getUserFromToken(req);
    if (!user) {
      return res.status(401).json({ message: 'Access denied. Please login.' });
    ...
    // Delete all resolved panic alerts
    const { data, error } = await supabase
      .from('alerts')
      .delete()
      .eq('type', 'panic')
    ...
    res.status(200).json({
      message: `✅ Cleaned up ${data.length} resolved alerts`,
